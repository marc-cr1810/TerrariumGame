...
#include "Terrarium/Core/Timer.h"
...
namespace Terrarium
{
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			TE_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			TE_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		TE_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
